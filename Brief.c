...
void modifier(int index)
{
    if (index >= 0 && index < taille)
    {
        ...
        } while ((inf[index].date.jour < 1 || inf[index].date.jour > 31) ||
...
int main()
{
    ...
    do
    {
        ...
        case 3:
            do
            {
                printf("Entrer l'index de la tache a modifier (entre 1 et %d) : ", taille);
                scanf("%d", &index);

                if (index < 1 || index > taille)
                {
                    printf("Index invalide ! Veuillez reessayer.\n");
                }
            } while (index < 1 || index > taille);

            // Une fois un index valide entré
            modifier(index - 1);
            break;
